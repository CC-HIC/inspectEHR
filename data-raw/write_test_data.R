...
test_integer_2d$datetime <- seq(
  from = as.POSIXct("2018-01-01 12:00:01", format = "%Y-%m-%d %H:%M:%OS", tz = "GMT"),
...
temp_id <- anti_join(test_episode_length, test_real_2d) %>%
  select(episode_id) %>%
  pull()
...
temp_id <- anti_join(test_episode_length, test_string_2d) %>%
  select(episode_id) %>%
  pull()
...
save(test_episode_length, test_integer_1d, test_integer_2d, test_real_1d, test_real_2d, test_string_1d,
  test_string_2d, test_time_1d, test_date_1d, test_datetime_1d,
  file = "./tests/test_data.RData"
)
